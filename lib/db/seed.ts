...
const main = async () => {
  try {
    ...
    // Add orderNumber to each order based on index
    const ordersWithNumbers = orders.map((order, index) => ({
      ...order,
      orderNumber: `ORD${String(index + 1).padStart(6, '0')}`,
    }))
    const createdOrders = await Order.insertMany(ordersWithNumbers)
    console.log({
      createdUser,
      createdProducts,
      createdReviews,
  ...
  const items = [
    {
      ...
      vendorId: product1.vendorId.toString(),
      vendorName: product1.vendorName,
    },
    {
      clientId: generateId(),
      ...
      countInStock: product1.countInStock,
      vendorId: product2.vendorId.toString(),
      vendorName: product2.vendorName,
    },
    {
      clientId: generateId(),
      ...
      countInStock: product1.countInStock,
      vendorId: product3.vendorId.toString(),
      vendorName: product3.vendorName,
    },
  ]
